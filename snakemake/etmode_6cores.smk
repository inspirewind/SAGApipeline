...
rule all:
    input:
        expand("com_etmode/{gen}/braker/", gen=GEN_WITH_BAM_INDEX)

rule braker2:
    input:
        fna="com_etmode/{gen}/merge_fix.fna",
        bam="com_etmode/{gen}/VARUS.bam"
    output:
        directory("com_etmode/{gen}/braker/")
    threads: 6
    shell:
        "braker.pl --genome={input.fna} --species=com_1031et_{wildcards.gen} --bam={input.bam} --cores {threads} --min_contig=200 --workingdir=com_etmode/{wildcards.gen}/braker"
