...
rule all:
    input:
        expand("com_epmode/{gen}/braker/", gen=GEN_INDEX)

rule braker2:
    input:
        fa="com_epmode/{gen}/merge_fix.fna",
        prot_seq="/public/home/yaohuipeng/gene_pre/snakemake/odb10_plants.fasta"
    output:
        directory("com_epmode/{gen}/braker/")
    threads: 6
    shell:
        "braker.pl --genome={input.fa} --prot_seq={input.prot_seq} --species=com_1029ep_{wildcards.gen} --cores {threads} --min_contig=200 --workingdir=com_epmode/{wildcards.gen}/braker"
