...
rule all:
    input:
        expand("com_etpmode/{gen}/braker/", gen=GEN_WITH_BAM_INDEX)

rule braker2:
    input:
        fna="com_etpmode/{gen}/merge_fix.fna",
        bam="com_etpmode/{gen}/VARUS.bam",
        pro="/public/home/yaohuipeng/gene_pre/snakemake/odb10_plants.fasta"
    output:
        directory("com_etpmode/{gen}/braker/")
    threads: 6
    shell:
        "braker.pl --genome={input.fna} --etpmode --prot_seq={input.pro} --species=com_1031etp_{wildcards.gen} --bam={input.bam} --cores {threads} --min_contig=200 --workingdir=com_etpmode/{wildcards.gen}/braker"
